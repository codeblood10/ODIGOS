...
{
$branch_v = $_POST["branch"];
}
if(isset($_GET["branch"]))
{
$branch_v = $_GET["branch"];
}
if(isset($_POST['vp']))
...
<?php
	...
	$sql_recent = "SELECT * FROM profile WHERE year = {$year_v}  AND branch = {$branch_v} AND approved  = 1 ";
...
$per_page = 16;   // Per Page
	...
	$sql_b = "SELECT * FROM branch WHERE b_id = {$branch_v} ";
		$sql_res_b =  $con->query($sql_b);
		$sql_row = $sql_res_b->fetch_assoc();
		$bname = $sql_row["b_name"];

		$sql_y = "SELECT * FROM year WHERE y_id = {$year_v} ";
		$sql_res_y =  $con->query($sql_y);
		$sql_row = $sql_res_y->fetch_assoc();
		$yname = $sql_row["y_name"];
	?>
	<h1 align="center"><?php echo $yname ?> Year</h1>
	<h2 align="center"><?php echo $bname ?></h2>
	<br><br>
	<?php
	$sql="SELECT * FROM profile WHERE year = {$year_v}  AND branch = {$branch_v} AND approved  = 1 ORDER BY roll_no ASC LIMIT $per_page OFFSET $row_start ";
...
if($prev_page)
{
    echo " <li><a href='$_SERVER[SCRIPT_NAME]?Page=$prev_page&year=$year_v&branch=$branch_v'><b > Back</b></a></li> ";
}

for($i=1; $i<=$num_pages; $i++){
    if($i != $page)
    {
        /*echo "<li><a href='$_SERVER[SCRIPT_NAME]?Page=$&year=$year_v&branch=$branch_v'>$i</a> </li>";*/
    }
    else
    {
        /*echo "<li> $i </li>";*/
...
if($page!=$num_pages)
{
    echo "<li> <a href ='$_SERVER[SCRIPT_NAME]?Page=$next_page&year=$year_v&branch=$branch_v'><b>Next</b></a></li> ";
}
