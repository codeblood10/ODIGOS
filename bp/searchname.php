...
<br><br>
<?php
$sql_recent = "SELECT * FROM profile WHERE LOWER(name) LIKE LOWER('%".$ndsearch."%') AND approved  = 1 ";
...
$sql = "SELECT * FROM profile WHERE LOWER(name) LIKE LOWER('%".$ndsearch."%') ORDER BY roll_no DESC LIMIT $per_page OFFSET $row_start ";
...
if($prev_page)
{
    echo " <li><a href='$_SERVER[SCRIPT_NAME]?Page=$prev_page&nsearch=$ndsearch'><b > Back</b></a></li> ";
}

for($i=1; $i<=$num_pages; $i++){
    if($i != $page)
    {
        /*echo "<li><a href='$_SERVER[SCRIPT_NAME]?Page=$i&nsearch=$ndsearch'>$i</a> </li>";*/
    }
    else
    {
        /*echo "<li> $i </li>";*/
...
if($page!=$num_pages)
{
    echo "<li> <a href ='$_SERVER[SCRIPT_NAME]?Page=$next_page&nsearch=$ndsearch'><b>Next</b></a></li> ";
}
